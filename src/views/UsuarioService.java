...
public class UsuarioService {
   ...
   public boolean registrarUsuario(String nome, String email,
    ...
    public String listarRendasDespesasPorPeriodo(String inicio, String fim) {
        ...
        List<Renda> rendas = new RendaDAO().listarRendasPorPeriodo(
                idUsuario,
                dataInicio,
                dataFim
        );

        List<Despesa> despesas = new DespesaDAO().listarDespesasPorPeriodo(
                idUsuario,
                dataInicio,
                dataFim
        );
        ...
        relatorio.append("Total de Rendas no período: R$ ").append(totalRendas).append("\n");
        relatorio.append("Total de Despesas no período: R$ ").append(totalDespesas).append("\n");
        relatorio.append("Saldo do Período: R$ ").append(saldo).append("\n");

        relatorio.append("\n--- RENDAS ---\n");
        if (rendas.isEmpty()) {
            relatorio.append("Nenhuma renda encontrada no período.\n");
        } else {
            for (Renda r : rendas) {
                relatorio.append("- ")
                        .append(r.getNomeRenda())
                        .append(" | R$ ")
                        .append(r.getValor())
                        .append(" | ")
                        .append(UtilData.formatarData(r.getData()))
                        .append("\n");
            }
        }
        ...
        relatorio.append("\n--- DESPESAS ---\n");
        if (despesas.isEmpty()) {
            relatorio.append("Nenhuma despesa encontrada no período.\n");
        } else {
            for (Despesa d : despesas) {
                relatorio.append("- ")
                        .append(d.getNomeDespesa())
                        .append(" | R$ ")
                        .append(d.getValor())
                        .append(" | ")
                        .append(UtilData.formatarData(d.getData()))
                        .append("\n");
            }
        }
